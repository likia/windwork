...
class Config extends Object{
	 ...
	 * 
	 */
	public function setConfig($config, $isAppend = TRUE){
		if ($isAppend) {
...
class Common_util_pub extends Config{
	...
	public function postXmlCurl($xml, $url, $second=30){		
		...
		if($data){
			...
			$error = curl_errno($ch);
			$this->setErr("curl出错，错误码:{$error}"); 
			$this->setErr("<a href='http://curl.haxx.se/libcurl/c/libcurl-errors.html'>错误原因查询</a>");
			return false;
		}
		return false;
	}
	...
	function createXml(){
		try{
			...
			if($this->parameters["out_trade_no"] == null) {
		   	...
		   	$this->parameters["spbill_create_ip"] = \core\App::getInstance()->getRequest()->getClientIp();//终端ip	    
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    //logging('debug', $this->parameters);
		    return  $this->arrayToXml($this->parameters);
...
// 		logging('debug', 'after postXml');
		$this->result = $this->xmlToArray($this->response);
		if (!$this->result || $this->result['return_code'] == 'FAIL') {
			$this->setErr($this->result['return_msg']);
			return false;
		}
// 		logging('debug', $this->result);
		if (empty($this->result['result_code']) || $this->result['result_code'] == 'FAIL') {
			$this->setErr('获取预支付ID失败');
			return false;
		}
		
		$prepay_id = $this->result["prepay_id"];
// 		logging('debug', $prepay_id);
		return $prepay_id;
	}
...
class Wxpay_server_pub extends Common_util_pub 
{
	...
	function setReturnParameter($parameter, $parameterValue){
		$this->returnParameters[trim($parameter)] = trim($parameterValue);
	}
...
class JsApi_pub extends Common_util_pub
{
	...
	function getOpenid(){
		...
		if (!$data || empty($data['openid'])) {
			$this->setErr('无法获取openid');
			return false;
		}

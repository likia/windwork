...
class Common {
	...
	/**
	 * 取得修正的$_SERVER['PHP_SELF']
	 * @return string
	 */
	public static function fixedPhpSelf() {
		static $phpself = null;
		if (!$phpself) {
			if (defined('PHP_SELF')) {
				$phpself = $_SERVER['PHP_SELF'] = PHP_SELF;
				return $phpself;
			}
			
			if(isset($_SERVER['PHP_SELF'])) {
				if($_SERVER['SCRIPT_FILENAME'] == $_SERVER['PHP_SELF']) {
					if(!empty($_SERVER['DOCUMENT_ROOT']) && strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) === 0) {
						$_SERVER['PHP_SELF'] = str_replace('\\','/', str_replace($_SERVER['DOCUMENT_ROOT'], '', $_SERVER['SCRIPT_FILENAME']));
					} else {
						$_SERVER['PHP_SELF'] = '/'.basename($_SERVER['SCRIPT_FILENAME']);
					}
				} elseif (strlen($_SERVER['SCRIPT_FILENAME']) < strlen($_SERVER['PHP_SELF'])) {
					$_SERVER['PHP_SELF'] = '/'.basename($_SERVER['SCRIPT_FILENAME']);
				}
			} else {
				$_SERVER['PHP_SELF'] = '/'.basename($_SERVER['SCRIPT_FILENAME']);
	...
	/**
	 * 获取修正后的$_SERVER['REQUEST_URI']
	 * 在部分web服务器上会没有REQUEST_URI变量
	 * 
	 * @return string
	 */
	public static function fixedRequestUri() {
		static $uri = null;
		
		if(!$uri) {
			$queryString = PHP_SAPI == 'cli' && isset($_SERVER['argv'][1]) ? $_SERVER['argv'][1] : @$_SERVER['QUERY_STRING'];
			if (!isset($_SERVER['REQUEST_URI'])) {
				$_SERVER['REQUEST_URI'] = self::fixedPhpSelf() . ($queryString ? '?'  : '') . $queryString;
	 ...
	 */
	public static function userProtocol() {
	    static $protocol = '';
		if($protocol) {
			return $protocol;
		}
	...
	public static function getDomain() {
		...
		return $domain = $protocol . $host . $port;
	}

	/**
	 * 当前请求的路径（相对于根目录的路径，域名根入口文件之间的字符串）
	 * example: http://www.windwork.org/xx/ 的上下文为 /xx/
	 *
	 * @return string
	 */
	 ...
	 * @return string
	 */
	public static function httpCodeMap($code) {
		$maps = array(
			100 => 'continue',

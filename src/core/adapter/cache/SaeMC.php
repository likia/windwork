 ...
 * @since       1.0.0
 */
class SaeMC extends ACache implements ICache, \core\adapter\IFactoryAble {
	/**
	 * 锁定
	 *
	 * @param string $key
	 * @return \core\adapter\cache\SaeMC
	 */
	...
	private function getCachePath($key) {
		...
		return $this->cacheDir . '/' . ltrim($key, '/') . '.php';
	}
	...
	public function read($key, $checkExpired = true) {
		...
		if (is_file($cachePath) && is_readable($cachePath)) {
			$data = substr(file_get_contents($cachePath), 13);
	...
	private function store($key, $value) {
	    ...
	    return @file_put_contents($cachePath, '<?php exit;?>'. $value);
	}
